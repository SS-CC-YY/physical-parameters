...
`center_10f` extracts the orange ball center in each previous frame and draws small center markers on the final frame. This is less visually invasive than `trail_10f`, which overlays faint translucent object silhouettes and can interfere with the generated object. The default `PROMPT_MODE=visual_trace` prompt does not include the hidden numeric target; it tells the model that the blue/red dots are motion-history annotations and must disappear in the generated video.
...
- Both still collapse: Wan2.2-I2V is not reliably extracting the hidden physical parameter from single-image visual conditioning.
- `PROMPT_MODE=explicit` remains available only as a leakage/control baseline because it puts the hidden target value in text.
