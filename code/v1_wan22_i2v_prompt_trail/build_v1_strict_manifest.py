...
def hidden_or_explicit(target: str, value: float, unit: str, prompt_mode: str) -> str:
    if prompt_mode == "hidden":
        return (
            "No numeric value for the hidden physical setting is provided. Infer the motion from the "
            "conditioning image and its motion-history annotation, then keep the continuation "
            "physically consistent. "
        )
    if prompt_mode == "visual_trace":
        return (
            "No numeric physical value is provided. Use only the visible object state and the "
            "motion-history annotation in the conditioning image to infer how the scene should "
            "continue. Do not assume a standard value from text. "
        )
    unit_text = f" {unit}" if unit else ""
    return f"The target physical parameter is {target} = {value:g}{unit_text}. "


def gravity_description(value: float | None) -> str:
    if value is None:
        return (
            "The ball continues vertical free fall with one constant downward acceleration implied "
            "by the recent center positions. Its downward speed must increase smoothly every frame. "
            "There is no horizontal drift, no spin, no air drag, and no bounce before floor contact. "
            "After contacting the floor, the ball stays still on the floor. "
        )
    if value < 5.0:
...
def restitution_description(value: float | None) -> str:
    if value is None:
        return (
            "At every floor impact, the same unknown restitution setting is used. Infer it from the "
            "recent motion-history annotation. The ball moves only vertically, and successive bounce "
            "heights should follow one consistent decay pattern. Do not introduce horizontal drift. "
        )
...
def friction_description(value: float | None) -> str:
    if value is None:
        return (
            "The ball slides horizontally to the right while staying at the same height. Infer the "
            "constant deceleration from the recent motion-history annotation. Its horizontal speed "
            "decreases smoothly and uniformly. There is no bounce, no vertical motion, no hop, and "
            "no camera movement. If the speed reaches zero, the ball remains still. "
        )
    decel = value * 9.81
    if value < 0.08:
...
def damping_description(value: float | None) -> str:
    if value is None:
        return (
            "The pendulum swings only in the image plane with one consistent damping setting inferred "
            "from the recent motion-history annotation. The pivot stays fixed, the rod is rigid, the "
            "rod length is constant, and the orange ball remains attached to the rod. There is no "
            "out-of-plane motion. "
        )
    if value < 0.08:
...
def make_prompt(experiment: str, value: float, camera: str, prompt_mode: str, conditioning_mode: str) -> str:
    ...
    visible_value = value if prompt_mode == "explicit" else None
    ...
    if experiment == "v1_A":
        prompt += gravity_description(visible_value)
    elif experiment == "v1_B":
        prompt += "Known gravity is 9.81 meters per second squared downward. " + restitution_description(visible_value)
    elif experiment == "v1_C":
        prompt += "Known gravity is 9.81 meters per second squared downward. " + friction_description(visible_value)
    elif experiment == "v1_D":
        prompt += "Known gravity is 9.81 meters per second squared downward. " + damping_description(visible_value)
    else:
