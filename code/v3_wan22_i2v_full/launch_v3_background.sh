...
OUTDIR="${OUTDIR:-outputs/v3_wan22_i2v_a14b}"
GPU_ID="${GPU_ID:-7}"
SIZE="${SIZE:-832*480}"
FRAME_NUM="${FRAME_NUM:-121}"
PROMPT_MODE="${PROMPT_MODE:-explicit}"
CAMERAS="${CAMERAS:-CAM_Side}"
EXPERIMENTS="${EXPERIMENTS:-v3_A v3_B v3_C v3_D v3_E v3_F v3_G}"
MAX_JOBS="${MAX_JOBS:-}"
...
python code/v3_wan22_i2v_full/build_v3_manifest.py \
...
RUN_CMD=(
  python code/v3_wan22_i2v_full/run_v3_wan22_official.py
  --manifest "${MANIFEST}"
  --wan-repo "${WAN_REPO}"
  --ckpt-dir "${CKPT_DIR}"
  --outdir "${OUTDIR}"
