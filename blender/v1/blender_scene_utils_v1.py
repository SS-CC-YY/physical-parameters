...
def build_base_world(label_text):
    ...
    add_plane("ENV_Floor", (0.0, 0.0, 0.0), 40, collection=env, material=mats["floor"])
    wall_height = 7.0
    wall_center_z = wall_height
    add_cube("ENV_BackWall", (0.0, 10.0, wall_center_z), (18.0, 0.10, wall_height), collection=env, material=mats["wall"])
    add_cube("ENV_LeftWall", (-18.0, 0.0, wall_center_z), (0.10, 10.0, wall_height), collection=env, material=mats["wall"])
    add_cube("ENV_RightWall", (18.0, 0.0, wall_center_z), (0.10, 10.0, wall_height), collection=env, material=mats["wall"])

    camera_target = (0.0, 0.0, 1.3)
    cam_main_location = (6.0, -13.5, 4.8)
    cam_main = add_camera("CAM_Main", cam_main_location, (74, 0, 0), cameras, lens=48)
    look_at(cam_main, camera_target)
    cam_side = add_camera("CAM_Side", (0.0, -16.0, 3.8), (84, 0, 0), cameras, lens=56)
    # Keep the historical camera name for compatibility, but define CAM_Top as
    # the exact y-axis mirror of CAM_Main so cross-view comparisons stay paired.
    ...
    add_area_light("LIGHT_Key", (7.0, -7.0, 8.8), (58, 0, 34), 2800, 8.0, (1.0, 0.96, 0.92), lights)
    add_area_light("LIGHT_Fill", (-5.5, -8.8, 5.9), (70, 0, -24), 1200, 6.0, (0.84, 0.9, 1.0), lights)
    add_area_light("LIGHT_Top", (0.0, 0.0, 11.0), (180, 0, 0), 1400, 10.0, (1.0, 1.0, 1.0), lights)
...
def print_summary(names):
    print("SUMMARY")
    for frame in (1, 24, 48, 96, 144, 192):
        bpy.context.scene.frame_set(frame)
        print(f"FRAME {frame}")
        for name in names:
