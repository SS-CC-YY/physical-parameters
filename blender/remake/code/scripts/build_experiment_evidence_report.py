...
def _paper_main_table_markdown(
    ...
    lines = [
        ...
        "",
        "| 模型 | Eq. support | Acc@25 | Median BNAE† | Fit R² / NRMSE† | Dir./all | Scan coverage | 实验结论 | 待重拟合实验 | 真U实验 |",
        "|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|",
    ]
    for row in rows:
        lines.append(
    ...
    lines.extend(
        [
            ...
            "",
            "“待重拟合实验”和“真U实验”是 13 个实验中的实验级单元数，不是视频数；逐视频数量及比例见 `paper_evidence_model_summary.csv` 的 `pending_refit_job_count`、`genuine_u_job_count` 和 `genuine_u_job_fraction`。",
            "",
            "## 指标口径",
            "",
...
def _markdown_report(
    ...
    lines = [
        ...
        "",
        "- `A`：方向与数值均准确；`D`：至少一个参数分量方向正确但数值不准（组合实验不等于全部分量同时正确）；`N`：有数值轨迹证据但参数扫描无正确响应。",
