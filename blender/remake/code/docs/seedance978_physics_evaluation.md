...
- Main/Top 的 `borderline_below_threshold` 不再无条件进入动态分支：只有状态正常、无切镜、P95 直接平移 `<=1%` 对角线/旋转 `<=0.75°`/缩放 `<=3%`，链式累计 P95 平移 `<=2%`/旋转 `<=1°`/缩放 `<=5%`，配准残差 `<=0.2%` 对角线、有效配对率 `>=0.80`、中位内点率 `>=0.65`，并且三类阈值命中数与持续运动簇都为零，才以 `calibrated_motion_within_tolerance` 走标定 2D；
- 明确且持续的 Main/Top 平移、变焦或旋转仍进入动态分支。该判断只表示冻结标定已不适用；动态 3D 结果仍需通过独立的背景锚点、刚体与重投影质量门；
...
三视角合计为 `975` 条标定 2D 与 `3` 条动态 3D 候选。baseline Top 边界样本的直接和累计 P95 都很小，按统一证据门回到 2D；indoor3 Top 虽然直接配准没有命中，但累计背景位移约 `9.86%` 对角线且接触表可见持续重构图，因此保留动态路线。
