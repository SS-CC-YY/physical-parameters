...
3. `review` / `borderline_below_threshold` 不再等同于“需要 3D”。只有状态正常、无切镜、背景配准质量合格，且直接平移/旋转/缩放均低于归一化容差、三类命中数和持续运动簇均为零时，才作为 `calibrated_motion_within_tolerance` 回到标定 2D；否则仍进入动态分支。
4. 审计错误、证据缺失或文件名不匹配不会套用静态相机参数。当前兼容流程仍保守地交给动态分支，但不会把该路由本身当作“3D 已可靠”的证据；SpatialTrackerV2 的轨迹还必须通过独立质量门。

这里的阈值按图像对角线归一化，不绑定 Seedance 分辨率。当前 Main/Top 的边界级 2D 容差要求两组证据同时通过：P95 直接平移 `<=1%` 对角线、旋转 `<=0.75°`、缩放 `<=3%`；链式累计 P95 平移 `<=2%`、旋转 `<=1°`、缩放 `<=5%`；另要求配准残差 `<=0.2%` 对角线、有效配对率 `>=0.80`、中位内点率 `>=0.65`、零阈值命中、零持续簇和零切镜。直接配准避免链式漂移误报，累计配准则避免漏掉平滑跟拍/变焦。它只说明冻结标定误差仍在可接受范围内，不表示超过阈值就天然具有可靠 3D 视差。
